...
function SimuladorHover(t,lim,x,x0,y,y0,psi)
    ...
    for i = 1:100:length(t)
        ...
        Cylinder([y(i),x(i),0],[y(i)+(comp*sin(psi(i))),x(i)+(comp*cos(psi(i))),0],2,20,[1,0,0],1,0);
        Cylinder([y(i),x(i),0],[y(i)+(comp*cos(psi(i))),x(i)-(comp*sin(psi(i))),0],2,20,[0,1,0],1,0);
        ...
        if i == length(t)
            for j = 2:i  
                hold on;
                plot3([y(j) y(j-1)],[x(j) x(j-1)],[-5.1 -5.1],'r-','LineWidth',2)
            end
        end
        pause(0.005);
    end
